...
/// The raw test harness: status readouts and one button per SDK path (start each template, update,
/// end all, the stale demo, the second activity for the minimal presentation). The Trips tab is the
/// presentation face; this tab exists to exercise every flow directly.
...
#Preview {
    DeveloperView(controller: LiveActivityController())
}
