---
title: Widget Extension setup
description: Add the thin Widget Extension that renders LiveStage Live Activities.
---

A Live Activity is rendered by a Widget Extension, a separate target from your app. LiveStage ships
the native views in the `LiveStageUI` package, so your extension is thin: it registers one widget and
links two packages. The `TripDemoWidget` target in `ios-demo/` is the reference starter; copy its
shape.

:::note[The extension has no network or location]
...
1. **Add the target.** In Xcode: File > New > Target > Widget Extension. Give it a name (for example
   `MyAppWidget`). Xcode creates a target with its own Info.plist and a sample widget; you replace the
   sample with the bundle above.
...
2. **Link the packages.**
   ...
   The shared `LiveStageActivityAttributes` type lives in `LiveStageModels`, imported by both the app
   and the extension, so there is a single definition of the activity's attributes on both sides.
...
Build and run the app on a simulator or device, then start an activity from your code (see
[Getting started](/LiveStage/getting-started/)). The Lock Screen card appears on any simulator. The
Dynamic Island only appears on a Pro simulator or device, so do not conclude rendering is broken from
a non-Pro simulator. The minimal Dynamic Island presentation only appears when two or more activities
are live at once.
